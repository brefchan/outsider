...
// 删除文件的操作
func (conf *HadeConfig) removeConfigFile(file string) error {
	conf.lock.Lock()
	defer conf.lock.Unlock()
...
func NewHadeConfig(params ...interface{}) (interface{}, error) {
	...
	// 初始化应用路径
	if hadeConfig.IsExist("app.path") && c.IsBind(contract.AppKey) {
		appPaths := hadeConfig.GetStringMapString("app.path")
			...
			case ev := <-watch.Events:
				{
					...
					if ev.Op&fsnotify.Create == fsnotify.Create {
						...
						hadeConfig.loadConfigFile(folder, fileName)
					}
					...
					if ev.Op&fsnotify.Remove == fsnotify.Remove {
						...
						hadeConfig.removeConfigFile(fileName)
					}
				}
