...
const (
	// UnknownLevel 表示位置的日志级别
	UnknownLevel LogLevel = iota
	// PanicLevel 表示会导致整个程序出现崩溃的日志信息
	PanicLevel
	// FatalLevel 表示会导致当前这个请求出现提前终止的错误信息
	FatalLevel
	// ErrorLevel 表示出现错误,但是不一定影响后续请求逻辑的错误信息
	ErrorLevel
	// WarnLevel 表示出现错误,但是一定不影响后续请求逻辑的报警信息
	WarnLevel
	// InfoLevel 表示正常的日志信息输出
	InfoLevel
	// DebugLevel 表示在调试状态下打印出来的日志信息
	DebugLevel
	// TraceLevel 表示最详细的信息,一般信息量比较大,可能包含调用堆栈等信息
	TraceLevel
)
...
type Log interface {
	...
	Error(ctx context.Context, msg string, field map[string]interface{})
	...
	Trace(ctx context.Context, msg string, field map[string]interface{})
	// SetLevel 设置日志级别
